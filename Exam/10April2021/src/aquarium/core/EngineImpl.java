...
public class EngineImpl implements Engine {
    ...
    private String processInput() throws IOException {
        ...
        switch (command) {
            case AddAquarium:
                result = controller.addAquarium(data[0], data[1]);
                break;
            case AddDecoration:
                result = controller.addDecoration(data[0]);
                break;
            case InsertDecoration:
                result = controller.insertDecoration(data[0], data[1]);
                break;
            case AddFish:
                result = controller.addFish(data[0], data[1], data[2], data[3], Double.parseDouble(data[4]));
                break;
            case FeedFish:
                result = controller.feedFish(data[0]);
                break;
            case CalculateValue:
                result = controller.calculateValue(data[0]);
                break;
            case Report:
