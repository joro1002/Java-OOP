...
public class ControllerImpl implements Controller {
    ...
    public String retireAstronaut(String astronautName) {
        if (astronautRepository.getModels().stream().noneMatch(a -> a.getName().equals(astronautName))) {
    ...
    public String explorePlanet(String planetName) {
        List<Astronaut> astronautsOxygenOver60 = this.astronautRepository.getModels().stream()
                .filter(a -> a.getOxygen() > 60).collect(Collectors.toList());

        if (astronautsOxygenOver60.isEmpty()) {
            throw new IllegalArgumentException(ExceptionMessages.PLANET_ASTRONAUTS_DOES_NOT_EXISTS);
        }

        int countBeforeMission = astronautsOxygenOver60.size();
        Mission mission = new MissionImpl();
        Planet planet = planetRepository.findByName(planetName);
        mission.explore(planet, astronautsOxygenOver60);
        countExploredPlanets++;
        int countAfterMission = astronautsOxygenOver60.size();
