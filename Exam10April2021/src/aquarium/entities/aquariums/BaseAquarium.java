...
public abstract class BaseAquarium implements Aquarium {
    ...
    public String getInfo() {
        String outputFist = fish.isEmpty() ? "none" : fish.stream().map(Fish::getName).collect(Collectors.joining(" "));

        return String.format("%s (%s):%n" +
                "Fish: %s%n" +
                "Decorations: %d%n" +
                "Comfort: %d", name, this.getClass().getSimpleName(), outputFist, decorations.size(), calculateComfort());
    }
